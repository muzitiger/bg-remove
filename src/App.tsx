...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-surface" onPaste={handlePaste}>
      {/* Nav */}
      <nav className="bg-surface-1/80 backdrop-blur-xl border-b border-border sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5">
          <div className="flex items-center justify-between">
            <h1 className="text-xl font-semibold tracking-tight text-on-surface flex items-center gap-2">
              <span className="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-accent-dim text-accent text-sm font-bold">B</span>
              PixClear
            </h1>
            {!isIOS && (
              <div className="flex items-center gap-3">
                <span className="text-sm text-on-surface-dim hidden sm:inline">Model</span>
                <select
                  value={currentModel}
                  onChange={handleModelChange}
                  className="bg-surface-2 border border-border rounded-lg px-3 py-1.5 text-sm text-on-surface focus:outline-none focus:ring-2 focus:ring-accent-ring focus:border-accent/40 hover:border-border-hover cursor-pointer appearance-none"
                  disabled={!isWebGPU}
                >
          ...
          {isIOS && (
            <p className="text-sm text-on-surface-faint mt-1.5">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className={`grid ${images.length === 0 ? 'grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12' : 'grid-cols-1'}`}>
          {images.length === 0 && (
            <div className="flex flex-col justify-center items-start animate-fade-in order-2 lg:order-1">
              <img 
                src="hero.png"
                alt="Surprised man"
                className="mb-8 w-full object-cover h-[280px] sm:h-[360px] rounded-2xl shadow-card"
              />
              <h2 className="text-3xl sm:text-4xl font-bold text-on-surface mb-3 tracking-tight leading-tight">
          ...
          <div className={`${images.length === 0 ? 'order-1 lg:order-2' : 'w-full'}`}>
            {/* Full dropzone — only when no images */}
            {images.length === 0 && (
              <div
                {...getRootProps()}
                className={`p-8 sm:p-10 mb-8 border-2 border-dashed rounded-2xl text-center cursor-pointer group
                  ${isDragAccept ? "border-success bg-success-dim" : ""}
                  ${isDragReject ? "border-danger bg-danger-dim" : ""}
                  ${isDragActive ? "border-accent bg-accent-dim shadow-glow" : "border-border hover:border-accent/40 hover:bg-accent-dim/50 hover:shadow-glow"}
                  ${isLoading || isModelSwitching ? "cursor-not-allowed opacity-70" : ""}
                `}
              >
                ...
                <div className="flex flex-col items-center gap-3">
                  {isLoading || isModelSwitching ? (
                    <>
                      <div className="inline-block animate-spin rounded-full h-10 w-10 border-2 border-accent/30 border-t-accent mb-1"></div>
                      <p className="text-base text-on-surface-dim">
                    ...
                    <>
                      ...
                      <p className="text-base text-danger font-medium mb-1">{error.message}</p>
                      {currentModel === 'Xenova/modnet' && (
                        <button
                          ...
                          className="px-4 py-2 bg-accent text-white text-sm font-medium rounded-xl hover:bg-accent-hover active:scale-[0.97]"
                        >
                    ...
                    <>
                      <div className="w-12 h-12 rounded-xl bg-accent-dim flex items-center justify-center group-hover:scale-110 transition-transform duration-200">
                      ...
                      <p className="text-sm text-on-surface-faint">or click to select files</p>
                    </>
                  )}
                </div>
              </div>
            ...
            {/* Sample images — only when no images */}
            {images.length === 0 && (
              <div className="bg-surface-1 rounded-2xl p-6 shadow-card animate-slide-up">
                <h3 className="text-base text-on-surface font-medium mb-4">No image? Try one of these</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  {sampleImages.map((url, index) => (
                    <button
                      key={index}
                      onClick={() => handleSampleImageClick(url)}
                      className="relative aspect-square overflow-hidden rounded-xl group focus:outline-none focus:ring-2 focus:ring-accent-ring focus:ring-offset-2 focus:ring-offset-surface-1"
                    >
                      <img
                        src={url}
                        alt={`Sample ${index + 1}`}
                ...
                <p className="text-xs text-on-surface-faint mt-4 flex items-center gap-1.5">
            ...
            {/* Compact add-more dropzone — only when images exist */}
            {images.length > 0 && (
              <div
                {...getRootProps()}
                className={`mt-4 p-4 border-2 border-dashed rounded-xl text-center cursor-pointer group
                  ${isDragAccept ? "border-success bg-success-dim" : ""}
                  ${isDragReject ? "border-danger bg-danger-dim" : ""}
                  ${isDragActive ? "border-accent bg-accent-dim" : "border-border hover:border-accent/40 hover:bg-accent-dim/50"}
                `}
              >
                <input {...getInputProps()} className="hidden" />
                <div className="flex items-center justify-center gap-2">
                  <svg className="w-5 h-5 text-on-surface-faint group-hover:text-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4" />
                  </svg>
                  <span className="text-sm text-on-surface-dim group-hover:text-on-surface">Add more images</span>
                </div>
              </div>
            )}
          </div>
